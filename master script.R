...
ggplot(fig3E, aes(x = dscore, fill=condition, alpha=condition, linetype=condition)) +
  ...
  geom_vline(xintercept = mean(subset(fig3E, condition=="Image")$dscore, na.rm=T), color="purple", size=2) + 
  geom_vline(xintercept = mean(subset(fig3E, condition=="Text")$dscore, na.rm=T), color="forestgreen", size=2) + 
  geom_vline(xintercept = mean(subset(fig3E, condition=="Control")$dscore, na.rm=T), color="black", size=2, linetype="dotted") + 
...
#Controls for Fig. 3C and Fig. 3D
fig3ACD$condition<-as.factor(as.character(fig3ACD$condition))
fig3ACD <- within(fig3ACD, condition <- relevel(condition, ref = 2))

#Fig.3C controls
table_s12_mod1<-lm(gender.rate ~ condition, data=fig3ACD)
summary(table_s12_mod1)
table_s12_mod1_vcov <- cluster.vcov(table_s12_mod1, fig3ACD$category)
coeftest(table_s12_mod1, table_s12_mod1_vcov)

table_s12_mod2<-lm(gender.rate ~ stim.parity + condition, data=fig3ACD)
summary(table_s12_mod2)
table_s12_mod2_vcov <- cluster.vcov(table_s12_mod2, fig3ACD$category)
coeftest(table_s12_mod2, table_s12_mod2_vcov)

table_s12_mod3<-lm(gender.rate ~ stim.parity * condition, data=fig3ACD)
summary(table_s12_mod3)
table_s12_mod3_vcov <- cluster.vcov(table_s12_mod3, fig3ACD$category)
coeftest(table_s12_mod3, table_s12_mod3_vcov)

#Fig.3D controls
table_s12_mod4<-lm(str_stereo ~ condition, data=fig3ACD)
summary(table_s12_mod4)
table_s12_mod4_vcov <- cluster.vcov(table_s12_mod4, fig3ACD$category)
coeftest(table_s12_mod4, table_s12_mod4_vcov)

table_s12_mod5<-lm(str_stereo ~ stim.str + condition, data=fig3ACD)
summary(table_s12_mod5)
table_s12_mod5_vcov <- cluster.vcov(table_s12_mod5, fig3ACD$category)
coeftest(table_s12_mod5, table_s12_mod5_vcov)

table_s12_mod6<-lm(str_stereo ~ stim.str * condition, data=fig3ACD)
summary(table_s12_mod6)
table_s12_mod6_vcov <- cluster.vcov(table_s12_mod6, fig3ACD$category)
coeftest(table_s12_mod6, table_s12_mod6_vcov)

#Robustness of Fig. 3F
exp_dt_main$ProlificID<-as.factor(exp_dt_main$ProlificID)
tables14<-lm(dscore ~ str_stereo + condition + category, data = exp_dt_main)
summary(tables14)
tables14_vcov <- cluster.vcov(tables14, exp_dt_main$ProlificID)
coeftest(tables14, tables14_vcov)
...
exp_dt_main_prime_ext$ProlificID<-as.factor(exp_dt_main_prime_ext$ProlificID)
mod_prime<-lm(str_stereo ~ condition + Obsv.Stereo.Str + Social.Category + ProlificID, data = exp_dt_main_prime_ext)
summary(mod_prime)
